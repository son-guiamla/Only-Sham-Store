...
<body>
  ...
  <script>
    ...
    function login() {
      ...
      .then(data => {
        if (data.success) {
          ...
          if (data.error.includes('admin code')) {
            showError('admin-code-error', data.error);
          }
        }
      })
      .catch(error => {
      ...
      .then(data => {
        if (data.success) {
          ...
          if (data.error.includes('username')) {
            showError('signup-username-error', data.error);
          } else if (data.error.includes('email')) {
            showError('signup-email-error', data.error);
          } else if (data.error.includes('phone')) {
            showError('signup-phone-error', data.error);
          } else {
            showError('signup-password-error', data.error || 'Registration failed');
          }
        }
      })
      .catch(error => {
        console.error('Signup error:', error);
        showError('signup-password-error', 'An error occurred during registration');
      });
    }
    ...
    function sendOTP() {
      ...
      .then(data => {
        if (data.success) {
          ...
          showError('forgot-phone-error', data.error || 'Failed to send OTP');
        }
      })
      .catch(error => {
        console.error('Error sending OTP:', error);
        showError('forgot-phone-error', 'Failed to send OTP');
      });
    }
    ...
    function resendOTP() {
      fetch('auth.php?action=resendOTP', {
        method: 'POST',
    ...
    function resetPassword() {
      ...
      fetch('auth.php?action=resetPassword', {
        method: 'POST',
        body: JSON.stringify({ 
          phone: resetPhone,
    ...
    window.onload = function() {
      ...
      // Show admin code field if username is admin
      ...
      document.getElementById('login-username').addEventListener('input', function() {
        document.getElementById('admin-code-group').style.display = 
          this.value.trim().toLowerCase() === 'admin' ? 'block' : 'none';
      });
